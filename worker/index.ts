...
async function listTools(request: Request, env: Env): Promise<Response> {
  ...
  const rows = await env.DB.prepare(`
    ...
    WHERE t.is_public = 1 AND (? = '%%' OR t.name LIKE ? OR t.type LIKE ? OR t.manufacturer LIKE ?)
    GROUP BY t.id, r.id
    ORDER BY vote_count DESC, t.name ASC
    LIMIT 100
  `).bind(user?.id || '', q, q, q, q).all<any>();
...
async function publishTool(request: Request, env: Env): Promise<Response> {
  ...
  await env.DB.batch([
    env.DB.prepare(`INSERT INTO tools (id, owner_user_id, name, type, units, diameter, flutes, v_angle, manufacturer, product_url, notes, source, is_public)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .bind(toolId, user.id, input.tool.name, input.tool.type, input.tool.units, input.tool.diameter, input.tool.flutes, input.tool.vAngle || 0, input.tool.manufacturer || '', input.tool.productUrl || '', input.tool.notes || '', input.tool.source || 'manual', input.tool.isPublic ? 1 : 0),
